...
import { Calendar, CheckCircle, AlertCircle, RefreshCcw } from 'lucide-react';
...
const History: React.FC = () => {
  ...
  const latestEntryId = historyEntries.length > 0 ? historyEntries[0].id : null;
      ...
      {historyEntries.length === 0 ? (
        ...
        <div className="space-y-6">
          {historyEntries.map((entry) => {
            ...
            return (
              <div key={entry.id} className={`bg-gray-900 border rounded-3xl overflow-hidden shadow-2xl group transition-all ${isLatest ? 'border-amber-500/30' : 'border-gray-800 hover:border-emerald-900/40'}`}>
                <div className={`p-6 flex flex-wrap items-center justify-between gap-4 border-b ${isLatest ? 'border-amber-500/10 bg-amber-500/[0.02]' : 'border-gray-800 bg-gray-800/10'}`}>
                  <div className="flex items-center gap-4">
                    <div className={`${isLatest ? 'bg-amber-500/10 text-amber-500' : 'bg-emerald-600/10 text-emerald-500'} p-3 rounded-2xl border ${isLatest ? 'border-amber-500/20' : 'border-emerald-900/20'}`}>
                      <Calendar size={24} />
                    </div>
                    <div>
